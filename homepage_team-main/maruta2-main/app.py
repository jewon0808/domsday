...
def login():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT classnumber, password FROM user WHERE classnumber=%s;", (classnumber))
        ...
        if result["classnumber"] == classnumber:
            if result["password"] == encoded_password:
                return jsonify(
                    access_token = create_access_token(identity = classnumber, expires_delta= False),
                    ok = True)
            else:
                return jsonify(
...
def next_page():
    ...
    return render_template('main.html', result_recs = result_recs)
...
def list_table_page(table, page):
    ...
    offset = (page - 1) * 10
    cursor.execute(f'SELECT id, title, article from {table} ORDER BY id DESC limit 10 offset {offset};')
    results = cursor.fetchall()
    return render_template('post_list.html', results = results, page = page, table = table, total_page = total_page)

   
# in post 
@app.route('/<table>/<title>/<int:id>')
def post(table, title, id):
    cursor.execute('USE post')
    cursor.execute(f'select title, article, image, recommand from `{table}`  WHERE title=%s', (title, ))
    result = cursor.fetchone()
    cursor.execute(f'SELECT nickname, date, article FROM {table}_comment WHERE id=%s;', (id,))
    comment_result = cursor.fetchall()
    return render_template('post.html', result = result, table = table, title=title,id = id, url_for = url_for, comment_result = comment_result)
...
def post_create(table):
    ...
    if file:
        ...
        if file.filename.rsplit('.', 1)[1].lower() not in allowed:
            ...
            file.save(file_path)
            cursor.execute(f"INSERT INTO `{table}` (title, article, image) VALUES('{title}', '{article}', '{file_path}')")
            db.commit()
            return redirect(url_for('list_table_page', table=table, page=1))
    else:
        cursor.execute(f"INSERT INTO `{table}` (title, article) VALUES('{title}', '{article}')")
        db.commit()
        return redirect(url_for('list_table_page', table = table, page = 1))
...
def comment(table, title, id):
    ...
    article = request.form.get('article')
    cursor.execute(f"INSERT INTO {table}_comment (id, nickname, password, title, article, date) VALUES({id}, '{nickname}', '{password}', '{article}', NOW())")
    db.commit()
...
#go to ilbe
@app.route('/<table>/<title>/<int:id>/recommand')
