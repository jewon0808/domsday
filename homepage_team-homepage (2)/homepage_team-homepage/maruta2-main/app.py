...
def post(table, title, id):
    ...
    result = cursor.fetchone()
    cursor.execute(f'SELECT comment_id, nickname, date, article FROM {table}_comment WHERE id=%s ORDER BY comment_id DESC;', (id,))
...
def post_create(table):
    ...
    if file:
        ...
        if file.filename.rsplit('.', 1)[1].lower() not in allowed:
            ...
            file.save(file_path)
            cursor.execute(f"INSERT INTO `{table}` (title, article, image, password, date) VALUES('{title}', '{article}', '{file_path}', '{password}', NOW())")
            db.commit()
            return redirect(url_for('list_table_page', table=table, page=1))
    else:
        cursor.execute(f"INSERT INTO `{table}` (title, article, password, date) VALUES('{title}', '{article}', '{password}', NOW())")
        db.commit()
        return redirect(url_for('list_table_page', table = table, page = 1))
...
@app.route('/<table>/<title>/<int:id>/<comment_id>/check', methods=['POST'])
def check_comment(table, title, id, comment_id):
    cursor.execute('USE post')
    request_password = request.form.get("password2")
    cursor.execute(f"SELECT password from {table}_comment where password=%s and comment_id=%s", (request_password, comment_id))
...
def post_password(table, title, id):
    if request.method == 'POST':
        ...
        cursor.execute(f'SELECT password from {table} where id=%s AND password=%s', (id,))
        password = cursor.fetchone()
